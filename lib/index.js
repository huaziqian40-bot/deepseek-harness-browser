...
const Config = z.object({
  ...
  /** Max screencast frame dimension. */
...
async function ensureBrowser(opts = {}) {
  ...
  manager = await BrowserManager.launch({
    headful: activeHeadful,
    userDataDir: process.env.DSH_BROWSER_PROFILE_DIR ?? join(process.env.HOME ?? "/home/sandbox", ".dsh", "browser-profile"),
    viewport
  });
...
function apply(ctx, config) {
  const widgets = readFileSync(fileURLToPath(new URL("../client/widget.js", import.meta.url)), "utf8");
  const publishHtml = readFileSync(fileURLToPath(new URL("../client/publish.html", import.meta.url)), "utf8");
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: (req, res) => {
      ...
      res.writeHead(200, {
        ...
        "content-disposition": 'attachment; filename="dsh-tool-browser-2.0.1.zip"',
