...
const Config = z.object({
  ...
  /** Broadcast cadence (ms) for the latest screencast frame. Frames are pushed
   *  as they arrive (this is the throttle floor, not a sampling timer), so a
   *  small value means snappier interaction. */
  frameIntervalMs: z.natural().default(40),
  /** Max screencast frame dimension — matched to a typical panel so the
   *  picture is not upscaled (blurry) and input mapping stays 1:1. */
  frameMaxDimension: z.natural().default(1280)
});
...
let lastFrame = null;
let lastFrameBuf = null; // decoded JPEG of lastFrame (shared by WS + MJPEG)
let lastPushAt = 0; // throttling clock for frame pushes
...
let viewMode = "view";
let profileDir = "";
/** Default page viewport, taken from the plugin config. */
let defaultViewport = { width: 1280, height: 800 };
/** Fixed DevTools port so a restarted server can re-attach to the SAME browser
 *  (keeps tabs, navigation history and logins alive across restarts). */
const BROWSER_DEBUG_PORT = Number(process.env.DSH_BROWSER_DEBUG_PORT ?? 9333);
/** URL for newly created tabs — the browser's own start page. `about:newtab`
 *  is blocked over CDP ("about:blank#blocked"), so use the vendor new-tab URL
 *  (Edge: edge://newtab/, Chromium: chrome://newtab/). Overridable via env. */
function newTabUrl() {
  if (process.env.DSH_BROWSER_NEW_TAB_URL) return process.env.DSH_BROWSER_NEW_TAB_URL;
  const bin = String(manager?.binary ?? "").toLowerCase();
  return bin.includes("edge") ? "edge://newtab/" : "chrome://newtab/";
}
...
async function ensureBrowser(opts = {}) {
  ...
  const viewport = {
    width: opts.width ?? defaultViewport.width,
    height: opts.height ?? defaultViewport.height
  };
  ...
  manager = await BrowserManager.launch({
    ...
    // Fixed DevTools port: after a dsh web restart we re-attach to the browser
    // that is still running there, so tabs/history/logins are preserved.
    port: BROWSER_DEBUG_PORT,
    viewport
  });
  manager.onFrame((frame) => {
    if (frame.type === "frame") {
      // Decode the base64 JPEG ONCE per frame and share the Buffer between the
      // WS broadcast and the MJPEG stream (it used to be decoded twice).
      const buf = Buffer.from(frame.data, "base64");
      lastFrame = { type: "frame", data: frame.data, width: frame.width, height: frame.height, timestamp: frame.timestamp };
      lastFrameBuf = buf;
      // Push immediately (throttled) instead of waiting for a timer tick —
      // the sampling interval used to add up to frameIntervalMs of pure lag
      // to every interaction.
      const now = Date.now();
      if (clients.size > 0 && now - lastPushAt >= frameIntervalMs) {
        lastPushAt = now;
        pushFrameToClients(buf, frame.width, frame.height);
      }
      if (mjpegClients.size > 0) pushMpeg(buf);
    } else {
      // browser exited
      lastFrame = null;
      lastFrameBuf = null;
  ...
  // Re-attaching to a still-running browser (after a server restart) restores
  // the user's tabs — push them to viewers right away.
  await broadcastTabsNow(manager);
...
/** Send one binary frame (9-byte header + JPEG) to every connected viewer. */
function pushFrameToClients(jpeg, width, height) {
  const header = Buffer.alloc(9);
  header[0] = 1; // binary frame marker
  header.writeUInt32BE(width, 1);
  header.writeUInt32BE(height, 5);
...
// Frames are pushed from the screencast callback the moment they arrive (see
// ensureBrowser). This timer is only a safety net for a static page whose
// screencast stops producing frames — it re-sends the latest frame now and
// then so a freshly connected viewer is never left staring at nothing.
function startBroadcast() {
  if (broadcastTimer) return;
  broadcastTimer = setInterval(() => {
    if (!lastFrameBuf || !lastFrame || clients.size === 0) return;
    const now = Date.now();
    if (now - lastPushAt < 500) return;
    lastPushAt = now;
    pushFrameToClients(lastFrameBuf, lastFrame.width, lastFrame.height);
  }, 250);
...
// The page itself can open/close tabs (target=_blank links, window.open, JS
// close) without any WS message — if we never noticed, the tab bar would stay
// stale and clicks on such links would look "dead". Poll the target list and
// broadcast whenever the set of tabs or the active tab changes.
let tabWatchTimer = null;
let lastTabSig = "";
function startTabWatch() {
  stopTabWatch();
  lastTabSig = "";
  tabWatchTimer = setInterval(async () => {
    const m = manager;
    if (!m) return;
    try {
      const tabs = await m.listTabs();
      const sig = `${m.pageTargetId}|${tabs.map((t) => t.id).join(",")}`;
      if (sig !== lastTabSig) {
        lastTabSig = sig;
  ...
  if (tabWatchTimer.unref) tabWatchTimer.unref();
}
function stopTabWatch() {
  if (tabWatchTimer) {
    clearInterval(tabWatchTimer);
    tabWatchTimer = null;
  }
}
...
function apply(ctx, config) {
    ...
    try {
      switch (msg.kind) {
        ...
        case "key": {
          ...
          } else if (params.op === "insertText") {
            // Composed text from the user's native IME (Chinese pinyin etc.)
            // — insert directly into the focused editable element.
            await m.session.send("Input.insertText", { text: params.text ?? "" });
          }
          break;
        }
        case "touch": {
        ...
        try {
          switch (msg.command) {
            ...
            case "resize": {
              ...
              // No-op when the viewport already matches: resize is re-requested
              // on every layout change, and re-applying an identical Emulation
              // override reflows the page for nothing (this starved tab
              // switching). Stay silent too — no toast per request.
              if (m.viewport && m.viewport.width === w && m.viewport.height === h) break;
        ...
        try {
          ...
          switch (msg.command) {
            case "list":
              ...
              break;
            case "new": {
              const tab = await m.openTab(msg.url ?? newTabUrl());
            ...
            case "close": {
              const tabs = await m.listTabs();
              // Closing the LAST tab: don't leave a zombie about:blank behind —
              // shut the whole shared browser down so the panel returns to its
              // idle "browser closed" state (matches user expectation).
              if (tabs.length <= 1) {
                manager = null;
                lastFrame = null;
                stopBroadcast();
                ...
                await m.close();
                broadcastStatus();
                sendNotice(wsClient, "已关闭最后一个标签，共享浏览器已停止");
              } else {
    ...
    const route = {
      ...
      handler: (req, socket, head) => {
        ...
        wss.handleUpgrade(req, socket, head, (wsClient) => {
          ...
          broadcastStatus();
          if (lastFrameBuf && lastFrame) {
            try {
              // reuse the already-decoded buffer (no extra base64 pass)
              const header = Buffer.alloc(9);
              header[0] = 1;
              header.writeUInt32BE(lastFrame.width, 1);
              header.writeUInt32BE(lastFrame.height, 5);
              wsClient.send(Buffer.concat([header, lastFrameBuf]));
            } catch {
              /* ignore */
            }
          }
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      url: { type: "string", description: "URL to open in the new tab (defaults to the browser's new-tab/start page)." }
    },
    output: {
      schema: { type: "object", additionalProperties: false, properties: {
    ...
    async execute(args) {
      const m = await requireBrowser();
      const tab = await m.openTab(args.url ?? newTabUrl());
      await m.switchToTab(tab.id);
      await restartScreencast(m);
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      const tabs = await m.listTabs();
      if (tabs.length <= 1) {
        // Last tab: shut the whole browser down (no zombie about:blank).
        manager = null;
        lastFrame = null;
        stopBroadcast();
        stopTabWatch();
        await m.close();
        broadcastStatus();
        return cleanJson({ closed: true, activeId: undefined, tabs: [] });
      }
      const closed = await m.closeTab(args.id);
      await restartScreencast(m);
      const remaining = await broadcastTabsNow(m);
      return cleanJson({ closed, activeId: m.pageTargetId, tabs: remaining });
    }
  }));

  // --- cleanup ---------------------------------------------------------------
  // DETACH (do not kill) the browser on unload: the process keeps running on
  // the fixed DevTools port with all its tabs/history, and the next start
  // re-attaches to it. Users explicitly close it via ⏹ / browser_close.
  ctx.effect(() => async () => {
    stopBroadcast();
    stopTabWatch();
    if (manager) {
